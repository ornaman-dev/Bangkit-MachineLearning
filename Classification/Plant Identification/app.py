...
if 'model' not in st.session_state:
    with st.spinner('Compiling & Load the model..'):
        st.session_state['model'] = load_model('Saved_Models/model_identification.h5',compile=False)
        st.session_state['model'].compile(optimizer = 'adam', 
                    loss = 'categorical_crossentropy', 
                    metrics = ['accuracy'])
        
        st.session_state['model_2'] = load_model('Saved_Models/model_EfficientNetB3.h5',compile=False)
        st.session_state['model_2'].compile(optimizer = 'adam', 
                    loss = 'categorical_crossentropy', 
                    metrics = ['accuracy'])
...
with col1:
    ...
    if uploaded_files is not None:
        ...
        with st.spinner('Predicting...'):
            ...
            pred = st.session_state['model'].predict(images,verbose=0)
    ...
    if uploaded_files is not None:
        ...
        if (pred_index == 1):
            with st.spinner('Further Predicting...'):
                 pred = st.session_state['model_2'].predict(images)
            
            predict_list = np.round(pred,3)
            if (np.max(predict_list) > 0.85):
